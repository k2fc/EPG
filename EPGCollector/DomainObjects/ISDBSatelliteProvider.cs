...
namespace DomainObjects
{
    ...
    public class ISDBSatelliteProvider : Provider
    {
        ...
        public static void Load()
        {
            ...
            string directoryName = Path.Combine(RunParameters.DataDirectory, "TuningParameters", "isdbs");
            DirectoryInfo directoryInfo;

            if (Directory.Exists(directoryName))
            {
                Logger.Instance.Write("Loading ISDB-S tuning files from " + directoryName);
            ...
            directoryName = Path.Combine(RunParameters.ConfigDirectory, "TuningParameters", "isdbs");
            Logger.Instance.Write("Loading ISDB-S tuning files from " + directoryName);
        ...
        {
            string path = Path.Combine(RunParameters.DataDirectory, "TuningParameters", "isdbs");
            ...
            try
            {
                ...
                foreach (ISDBSatelliteFrequency frequency in Frequencies)
                {
        ...
        {
            ISDBSatelliteProvider isdbProvider = provider as ISDBSatelliteProvider;
            ...
            if (Longitude != isdbProvider.Longitude)
                return (false);

            if (EastWest != isdbProvider.EastWest)
                return (false);

            if (level == EqualityLevel.Identity)
                return (true);

            if (Azimuth != isdbProvider.Azimuth)
                return (false);

            if (Elevation != isdbProvider.Elevation)
                return (false);
