...
namespace DomainObjects
{
    ...
    public class Bouquet
    {
        ...
        public static Collection<Bouquet> GetBouquetsInIdentityOrder()
        {
            Collection<Bouquet> sortedBouquets = new Collection<Bouquet>();

            foreach (Bouquet bouquet in Bouquets)
                addBouquetInIdentityOrder(sortedBouquets, bouquet);

            return (sortedBouquets);
        }

        private static void addBouquetInIdentityOrder(Collection<Bouquet> sortedBouquets, Bouquet newBouquet)
        {
            foreach (Bouquet oldBouquet in sortedBouquets)
            {
                if (oldBouquet.BouquetID > newBouquet.BouquetID)
