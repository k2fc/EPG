...
namespace DomainObjects
{
    ...
    public class Region
    {
        ...
        {
            TVStation newStation = TVStation.FindStation(RunParameters.Instance.StationCollection, 
                newChannel.OriginalNetworkID, newChannel.TransportStreamID, newChannel.ServiceID);
            if (newStation == null)
                newStation = new TVStation("No Name");

            foreach (Channel oldChannel in sortedChannels)
            {
                TVStation oldStation = TVStation.FindStation(RunParameters.Instance.StationCollection, 
                    oldChannel.OriginalNetworkID, oldChannel.TransportStreamID, oldChannel.ServiceID);
                if (oldStation == null)
