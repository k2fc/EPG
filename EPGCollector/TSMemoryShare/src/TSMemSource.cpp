...
HRESULT CTSMemSourcePin::GetMediaType(int iPosition, CMediaType *pmt)
{
	...
	pmt->InitMediaType();
	pmt->SetType      (& MEDIATYPE_MPEG2_SECTIONS);
	pmt->SetSubtype   (& MEDIASUBTYPE_DVB_SI);
...
HRESULT CTSMemSourcePin::CheckMediaType(const CMediaType *pMediaType)
{
	...
	if(MEDIATYPE_MPEG2_SECTIONS == pMediaType->majortype)
	{
		if (MEDIASUBTYPE_DVB_SI == pMediaType->subtype)
			return S_OK;
	}
...
CTSMemSourceFilter::CTSMemSourceFilter(IUnknown *pUnk, HRESULT *phr)
			: CSource(NAME("PSIMemorySourceFilter"), pUnk, CLSID_TSMemSourceFilter),
			m_pShareName(0),
			m_fileCurrentPos(-1),
			m_loopCount(-1),
...
HRESULT CTSMemSourceFilter::OpenLogFile(char *logFile)
{
	...
	m_hFile = CreateFile((LPCTSTR) "C:\\Temp\\PSIMemoryFilter.log",   // The filename
