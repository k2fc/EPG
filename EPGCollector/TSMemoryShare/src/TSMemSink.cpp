...
CTSMemSinkFilter::~CTSMemSinkFilter()
{
	...
	LogString("Destructor called - closing memory buffer\r\n");
	closeSharedMemory();
...
STDMETHODIMP CTSMemSinkFilter::Run(REFERENCE_TIME tStart)
{
	...
	if(isOpen() == false)
	{
		LogString("Creating memory buffer: size %d MB\r\n", sampleBufferSize);

		int createResult = createSharedMemory(sampleBufferSize);
...
CUnknown * WINAPI CTSMemSinkFilter::CreateInstance(LPUNKNOWN punk, HRESULT *phr)
{
    ...
    CTSMemSinkFilter *pNewObject = new CTSMemSinkFilter(punk, phr, false, "PSI Memory Filter.log", false, "PSI Memory Dump.ts", 50);
    if (pNewObject == NULL)
	{
        if (phr)
		{
...
{	
	...
	if (pidList[0] == -1)
	{
		...
		if((data->currentPointer + lDataLength) < getSharedDataSize())
		{
	...
	if (partSampleSize != 0)
	{
		...
		{			
			...
			dumpFileSize+= dwWritten;
			LogString("Dumped second part %d bytes at offset %d\r\n", 188 - partSampleSize, pbData);
		}
		else
		{			
			if((data->currentPointer + 188) < getSharedDataSize())
			{
		...
		{		
			...
			if (found)
			{
				...
				if (m_hDumpFile != INVALID_HANDLE_VALUE)
				{
					...
					dumpFileSize+= dwWritten;
					LogString("Sample dumped: offset %d size %d bytes\r\n", bufferPointer, 188);
				}
				else
				{
					if((data->currentPointer + 188) < getSharedDataSize())
	...
	if (bufferPointer != lDataLength)
	{
		...
		{
			DWORD dwWritten;
			WriteFile(m_hDumpFile, (PVOID)(pbData + bufferPointer), partSampleSize, &dwWritten, NULL);
			dumpFileSize+= dwWritten;
			LogString("Dumped part sample: size %d offset %d\r\n", partSampleSize, bufferPointer);
		}
		else
		{
