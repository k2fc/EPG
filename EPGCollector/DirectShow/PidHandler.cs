...
namespace DirectShow
{
    ...
    public class PidHandler
    {
        ...
        {
            if (TraceEntry.IsDefined(TraceName.PidHandlerBlocks))
                Logger.Instance.Dump(getPidID() + "Data Block", byteData, byteData.Length);
            if (TraceEntry.IsDefined(TraceName.PidHandler))
            ...
            if (TraceEntry.IsDefined(TraceName.PidHandlerSi))
        ...
        {
            return ("PID handler 0x" + pid.ToString("x") + " table 0x" + table.ToString("x") + ": ");
        }
    }
}
