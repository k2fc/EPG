...
namespace DirectShow
{
    ...
    {
        internal override string Description { get { return ("GenPix Open Source"); } }
        internal override bool CardCapable { get { return (cardCapable); } }

        private readonly Guid bdaTunerExtensionPropertiesOpenSource = new Guid(0x0B5221EB, 0xF4C4, 0x4976, 0xB9, 0x59, 0xEF, 0x74, 0x42, 0x74, 0x64, 0xD9);
        private readonly Guid bdaTunerExtensionPropertiesOfficial = new Guid(0xDF981009, 0x0D8A, 0x430E, 0xA8, 0x03, 0x17, 0xC5, 0x14, 0xDC, 0x8E, 0xC0);

        private readonly bool cardCapable;
        private readonly IKsPropertySet propertySet;

        private enum propertySetCommand
        {
            setFrequency,
            setDiseqc,
            getSignalStats
        ...
        private unsafe class DiseqcCommand
        {
        ...
        internal GenPixDiseqcHandler(IBaseFilter tunerFilter)
        {
            ...
            if (pin != null)
            {
                ...
                if (propertySet != null)
                {
                    ...
                    propertySet.QuerySupported(bdaTunerExtensionPropertiesOpenSource, (int)BdaTunerExtension.KSPROPERTY_BDA_DISEQC, out supported);
                    if ((supported & KSPropertySupport.Set) != 0)
                        cardCapable = true;
                    else
                    {
                        propertySet.QuerySupported(bdaTunerExtensionPropertiesOfficial, (int)BdaTunerExtension.KSPROPERTY_BDA_DISEQC, out supported);
                        if ((supported & KSPropertySupport.Set) != 0)
                            Logger.Instance.Write("GenPix official driver installed - set Diseqc handler to Legacy in EPG Centre");                            
                    }
                }
            }
        }
        ...
        /// <param name="diseqcRequest">The diseqc parameters.</param>  
        internal override bool SendDiseqcCommand(TuningSpec tuningSpec, string port, DiseqcRequest diseqcRequest)
        {
            if (!cardCapable)
                return (true);
        ...
        private bool sendCommand(byte[] command)
        {
            ...
            Logger.Instance.Write("GenPix DiSEqC handler (open source): sending command " + ConvertToHex(diseqcCommand.Command));

            reply = propertySet.Set(bdaTunerExtensionPropertiesOpenSource, (int)BdaTunerExtension.KSPROPERTY_BDA_DISEQC, replyBuffer, 32, commandBuffer, length);
            if (reply != 0)
                Logger.Instance.Write("GenPix DiSEqC handler (open source): command failed error code 0x" + reply.ToString("X"));
            else
                Logger.Instance.Write("GenPix DiSEqC handler (openSource): command succeeded");
