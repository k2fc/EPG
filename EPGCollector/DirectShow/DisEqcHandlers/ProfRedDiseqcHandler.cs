...
namespace DirectShow
{
    ...
    internal class ProfRedDiseqcHandler : DiseqcHandlerBase
    {
        ...
        private bool processPort(int lnbNumber, TuningSpec tuningSpec)
        {
            ...
            if (lnbNumber == 1)                                                   // No tone/data burst is the default
            ...
            byte[] commandBytes1 = GetCommand(lnbNumber, tuningSpec);
            
            bool commandReply = sendCommand(commandBytes1, modulationType, toneDataBurst);
            if (!commandReply)
                return (false);

            commandReply = sendCommand(commandBytes1, modulationType, toneDataBurst);
            if (!commandReply)
                return (false);

            byte[] commandBytes2 = GetSecondCommand(lnbNumber, tuningSpec);
            if (commandBytes2 == null)
                return (true);
            
            commandReply = sendCommand(commandBytes2, modulationType, toneDataBurst);
            if (commandReply)
                commandReply = sendCommand(commandBytes2, modulationType, toneDataBurst);                
