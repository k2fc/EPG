...
namespace DirectShow
{
    internal class TechnoTrendDiseqcHandler : DiseqcHandlerBase
    {
        ...
        {
            bool commandReply = sendCommand(GetCommand(lnbNumber, tuningSpec), Utils.GetNativePolarization(tuningSpec.SignalPolarization));
            if (!commandReply)
                return (false);
            ...
            return (sendCommand(commandBytes, Utils.GetNativePolarization(tuningSpec.SignalPolarization)));
        }
        ...
        private bool processCommands(string commands, TuningSpec tuningSpec)
        {
            ...
            foreach (string commandString in commandStrings)
            {
                ...
                bool reply = sendCommand(command, Utils.GetNativePolarization(tuningSpec.SignalPolarization));
                if (!reply)
                    return (false);
