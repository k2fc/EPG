...
namespace DirectShow
{
    ...
    public class PluginDataProvider : ISampleDataProvider
    {
        ...
        /// </summary>
        /// <param name="handle">Handle to a file or InvalidHandle to create memory objects.</param>
        /// <param name="lpAttributes">Security attributes. Set to null.</param>
        /// <param name="dwDesiredAccess">Access desired.</param>
        /// <param name="dwFileOffsetHigh">The memory size high.</param>
        /// <param name="dwFileOffsetLow">The memory size low.</param>
        ...
        /// </summary>
        public int DumpFileSize { get { return (0); } }
        ...
        {
            fileMapping = CreateFileMapping(new IntPtr(-1), 
                IntPtr.Zero, 
                (int)pageProtection.pageReadWrite, 
                0, 
                RunParameters.Instance.BufferSize * 1024 * 1024, 
                "DVBLogic Plugin Shared Memory " + runReference);
            if (fileMapping == null)
                return ("Failed to create file mapping");

            Logger.Instance.Write("Plugin data provider created shared memory with size " + RunParameters.Instance.BufferSize + "MB");
            ...
            Logger.Instance.Write("Plugin data provider mapped memory buffer");
