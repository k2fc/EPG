...
namespace DVBServices
{
    ...
    class SiehFernInfoEPGSection
    {
       /// <summary>
        ...
        /// </summary>
        public int SequenceCount { get { return (sequenceCount); } }
        ...
        public void Process(byte[] byteData, Mpeg2ExtendedHeader mpeg2Header)
        {
            ...
            tableIdExtension = mpeg2Header.TableIDExtension;
            ...
            try
            {
                ...
                sequenceCount = Utils.Convert2BytesToInt(byteData, lastIndex);
                lastIndex += 2;
        ...
        public void LogMessage()
        {
            ...
            Logger.ProtocolLogger.Write(Logger.ProtocolIndent + "SIEHFERN INFO EPG SECTION: Seq no:" + sequenceNumber +
                " seq count: " + sequenceCount + 
                " table ID ext: 0x" + tableIdExtension.ToString("x") + 
                " Unknown: " + Utils.ConvertToHex(unknown));
            if (DebugEntry.IsDefined(DebugName.SiehfernEpgDetail))
        ...
        internal static bool AddSection(SiehFernInfoEPGSection newSection)
        {
            ...
            {
                if (oldSection.TableIdExtension == newSection.TableIdExtension && oldSection.SequenceNumber == newSection.SequenceNumber)
                    return (false);

                if (oldSection.TableIdExtension > newSection.TableIdExtension)
                {
