...
namespace DVBServices
{
    ...
    public class OpenTVProgramCategory
    {
        ...
        /// <summary>
        /// Get or set the events for the category.
        /// </summary>
        public Collection<string> TraceEvents
        {
        ...
        private Collection<string> traceEvents;
        ...
        /// Collect trace data.
        /// </summary>
        /// <param name="eventDescription">The full description of the event.</param>
        public void SetTraceData(string eventDescription)
        {
            if (sampleEvent == null)
                sampleEvent = eventDescription;

            DebugEntry debugEntry = DebugEntry.FindEntry(DebugName.LogCatEvent);
            if (debugEntry == null)
                return;

            if (debugEntry.ParameterSet && CategoryID != debugEntry.Parameter)
                return;          

            if (traceEvents == null)
                traceEvents = new Collection<string>();
            traceEvents.Add(eventDescription);
        }
        ...
        public static void LogCategoryUsage()
        {
            ...
            if (categories != null)
            {
                foreach (OpenTVProgramCategory category in categories)
                {
                    if (category.TraceEvents != null)
                    {
                        Logger.Instance.WriteSeparator("Program Category " + category.CategoryID + " " + category.Description + " Events");

                        foreach (string eventDescription in category.TraceEvents)
                            Logger.Instance.Write(eventDescription);
                    }
                }
            }
        }
