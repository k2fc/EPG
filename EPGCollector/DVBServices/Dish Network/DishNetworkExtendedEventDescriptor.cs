...
namespace DVBServices
{
    ...
    /// </summary>
    internal class DishNetworkExtendedEventDescriptor : DishNetworkTextDescriptor
    {
        /// <summary>
        ...
        internal override void Process(byte[] byteData, int index)
        {
            try
            {
                originalDescription = Decompress(byteData, index);
            }
            ...
            int splitter = originalDescription.IndexOf((char)0x0d);
            if (splitter != -1)
            {
                subTitle = originalDescription.Substring(0, splitter);
                eventDescription = originalDescription.Substring(splitter + 2);                
            }
            else
        ...
        internal override void LogMessage()
        {
            ...
            Logger.ProtocolLogger.Write(Logger.ProtocolIndent + "DISH NETWORK EXTENDED EVENT DESCRIPTOR:" +
                " Description: " + (originalDescription == null ? "Not present" : originalDescription));
        }
    }
}
