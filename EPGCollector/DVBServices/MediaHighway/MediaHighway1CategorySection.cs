...
namespace DVBServices
{
    ...
    public class MediaHighway1CategorySection
    {
        ...
        internal void Process(byte[] byteData, int index)
        {
            ...
            try
            {
                while (descriptionIndex < byteData.Length)
                {
                    ...
                    categoryEntry.Description = Utils.GetString(byteData, descriptionIndex, 15, ReplaceMode.TransferUnchanged).Trim();
        ...
        {
            if (DebugEntry.IsDefined(DebugName.Mhw1CategorySections))
