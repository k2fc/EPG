...
namespace DVBServices
{
    ...
    public class MediaHighwayProgramCategory
    {
        ...
        public static bool Load(string fileName)
        {
            ...
            try { fileStream = new FileStream(fileNameUsed, FileMode.Open, FileAccess.Read); }
            catch (IOException e)
            {
                Logger.Instance.Write("Program category file " + fileNameUsed + " not available");
            ...
            {
                Logger.Instance.Write("Program category file " + fileNameUsed + " not available");
                fileNameUsed = Path.Combine(RunParameters.DataDirectory, "MHW Categories.cfg");
            ...
            Logger.Instance.Write("Loading MediaHighway Program Categories from " + fileNameUsed);
        ...
        public static void AddCategory(int number, string description)
        {
            ...
            foreach (MediaHighwayProgramCategory category in categories)
            {
                if (category.number == number)
                {
                    if (string.IsNullOrWhiteSpace(category.MHWDescription))
