...
namespace DVBServices
{
    ...
    internal class FreeSatChannelInfoDescriptor : DescriptorBase
    {
            ...
            try
            {
                if (Length != 2)
                {
                    ...
                    while (length > 0)
                    {
                        ...
                        if (channelInfoEntry.RegionEntries != null && channelInfoEntry.RegionEntries[0].ChannelNumber != -1)
        ...
        {
            logger.Write(Logger.ProtocolIndent + "FREESAT CHANNEL INFO DESCRIPTOR");

            Logger.IncrementProtocolIndent();

            if (channelInfoEntries != null && channelInfoEntries.Count != 0)
            {
                foreach (FreeSatChannelInfoEntry channelInfoEntry in channelInfoEntries)
                    channelInfoEntry.LogMessage();
            }
            else
                logger.Write(Logger.ProtocolIndent + "No entries present");
