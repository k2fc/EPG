...
namespace DVBServices
{
    ...
    internal class DVBSatelliteDeliverySystemDescriptor : DVBDeliverySystemDescriptor
    {
        ...
        /// </summary>
        public int ModulationSystem { get { return (modulationSystem); } }
        ...
        public override int Index
        {
            ...
            {
                if (lastIndex == -1)
                    throw (new InvalidOperationException("SatelliteDeliverySystemDescriptor: Index requested before block processed"));
                return (lastIndex);
            }
        }
        ...
        internal override void Process(byte[] byteData, int index)
        {
            ...
            {                
                ...
                modulationSystem = ((byteData[lastIndex] & 0x04) >> 2);
        ...
        {
            return (((decimal)OrbitalPosition / 10).ToString("0.0") + (EastFlag ? "E" : "W") + " " +
                (Frequency / 100) + decodePolarization(Polarization));
        }

        private string decodePolarization(int polarization)
        {
            switch (polarization)
            {
                case 0:
